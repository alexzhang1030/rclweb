...
- Process operations (`/livez`, `/readyz`, `/configz`, `/metrics`, `POST /drain`), the runtime images, the thin ament overlay (`ros2 run rclwebd rclwebd`, [deploy](../../docs/deploy.md#ros2-run)), and host systemd units (`packaging/systemd`, [deploy](../../docs/deploy.md#systemd)) are the deploy surface; `/healthz` stays liveness. Audit file-sink health lives on `/configz` / `/metrics`, not as event bodies. Kubernetes remains open. The overlay is not apt ([ADR 0018](../../docs/adr/0018-prebuilt-gateway-distribution.md)).
