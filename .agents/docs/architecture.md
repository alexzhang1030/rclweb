...
- The copy budget is one controllable payload copy end to end (RMW take). ROS_SAMPLE stays in the host WebSocket buffer; wasm is not on that data plane ([ADR 0017](../../docs/adr/0017-host-retain-inbound-sample-payload.md), [performance contracts](../../docs/architecture.md#performance-contracts)). Wasm-thread views are 0 copies; Worker→main host-retain String / PointCloud2 / generated corpus msg is a transfer (0 copies). The public `Node` API copies PointCloud2 `data`. Service/action CDR still copies at the Worker boundary. Foxglove / rosbridge: [performance](../../docs/performance.md).
...
- Process operations (`/livez`, `/readyz`, `/configz`, `/metrics`, `POST /drain`), the runtime images, and host systemd units (`packaging/systemd`, [deploy](../../docs/deploy.md#systemd)) are the deploy surface; `/healthz` stays liveness. Audit file-sink health lives on `/configz` / `/metrics`, not as event bodies. Kubernetes remains open.
