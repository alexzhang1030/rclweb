...
| Conformance | Fixtures (single oracle), corpus, and the support matrix |
...
- Process operations (`/livez`, `/readyz`, `/configz`, `/metrics`, `POST /drain`) and the Jazzy / Humble runtime images are the deploy surface; `/healthz` stays liveness. Audit file-sink health lives on `/configz` / `/metrics`, not as event bodies. Bloom / `packages.ros.org` stays deferred ([ADR 0018](../../docs/adr/0018-prebuilt-gateway-distribution.md)).
