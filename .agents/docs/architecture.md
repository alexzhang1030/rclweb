...
- Process operations (`/livez`, `/readyz`, `/configz`, `/metrics`, `POST /drain`), the runtime images, the thin ament overlay (`ros2 run rclwebd rclwebd`, [deploy](../../docs/deploy.md#ros2-run)), host systemd units (`packaging/systemd`, [deploy](../../docs/deploy.md#systemd)), the own apt repo (`rclwebd` / `rclweb-apt-source`, [ADR 0019](../../docs/adr/0019-own-apt-repository.md), [deploy](../../docs/deploy.md#apt)), and Kubernetes host-network units (`packaging/kubernetes`, [deploy](../../docs/deploy.md#kubernetes)) are the deploy surface; `/healthz` stays liveness. Audit file-sink health lives on `/configz` / `/metrics`, not as event bodies. The overlay is not bloom. Bloom / `packages.ros.org` stays deferred ([ADR 0018](../../docs/adr/0018-prebuilt-gateway-distribution.md)).
