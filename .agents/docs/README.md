...
rclweb is one Rust core (`rclweb`) serving the gateway natively and the browser as Wasm, a TypeScript package (`rcl-web` at `typescript/`), and R2WP over WebSocket and WebTransport. The host install the README and deploy page lead with is apt: `enable-rclweb-apt.sh`, then `apt install rclwebd` ([ADR 0019](../../docs/adr/0019-own-apt-repository.md)). Docker and `cargo install` are secondary. Do not delete `packaging/apt/` or `packaging/systemd/`.
...
| Host apt install, runtime images, operations endpoints | [Deploy](../../docs/deploy.md) (`#apt` first). Do not delete `packaging/apt/` / `packaging/systemd/` |
...
| `scripts/rosidl-dts.ts`, `typescript/rosidl/`, `typescript/src/cli.ts` | Users run `npx rcl-web gen` ([how to](../../docs/typescript.md#your-own-message-types)). Repo `--write`/`--check` vendor every Jazzy core `.msg`/`.srv`/`.action` (not `test_msgs`). `<=` bounds are not constants ([gotchas](./gotchas.md#ros-interface-bounds-are-not-constant-assignments)) |
...
| `packaging/apt/`, `packaging/systemd/`, `scripts/enable-rclweb-apt.sh`, `scripts/apt-*.ts`, `.github/workflows/publish-apt.yml` | Host install is apt ([ADR 0019](../../docs/adr/0019-own-apt-repository.md), [deploy](../../docs/deploy.md#apt)). The `.deb` packer needs the ament overlay + `rclwebd-ros.sh`. Do not delete these. [gotchas](./gotchas.md#do-not-delete-the-apt-repo-or-systemd-units) |
...
| `typescript/src/gateway-url.ts` | `init()` / loopback stay on WebSocket. `init("192.168.1.10")` → WebTransport (QUIC) on localhost; a LAN-IP page throws ([gotchas](./gotchas.md#intranet-webtransport-is-one-env-not-production-tls)) |
| `typescript/src/context.ts` | `init()` / `init(options)` / `init(url)` ; default `ws://127.0.0.1:8794/ws` |
