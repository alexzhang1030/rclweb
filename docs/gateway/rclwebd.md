...
`rclwebd` is rclweb's controlled edge boundary between browser sessions and ROS 2 domains. It links the shared [`rclweb` core](../runtime/core.md) for R2WP parsing and encoding, and owns transport, scheduling, sessions, schemas, policy, audit, telemetry, and recovery. ROS attachment goes through a versioned serialized adapter ABI ([ADR 0006](../adr/0006-edge-ros-c-abi-boundary.md)) with dynamic (dlopen) typesupport. Prebuilt images and binaries publish per release ([ADR 0018](../adr/0018-prebuilt-gateway-distribution.md), [deploy](../deploy.md)); the crate also publishes to crates.io ([release](../release.md)) — `cargo install rclwebd --features ros` needs a ROS prefix.
...
Default builds (`just check` / `just test` / `just build`) compile the gateway without ROS (`just check` also clippy's `--features webtransport` so the accept loop cannot rot): the rcl bindings are vendored (`rclwebd/src/ros/ffi/bindings.rs`, regenerated by `scripts/generate-rcl-bindings.sh`) and the protocol engine is tested against an in-memory backend. Builds with `--features ros` link core `rcl` / `rcl_action` / `rmw` / `rcutils` / `rosidl_runtime_c` from `ROS_PREFIX` / `AMENT_PREFIX_PATH` (default `/opt/ros/jazzy`); message/service/action typesupport libraries load at runtime via dlopen. The `rclwebd` binary requires the `ros` feature. Pair the prefix with `RCLWEBD_SUPPORT_ROW` (`J-*` ↔ Jazzy, `H-*` ↔ Humble). Committed bindings target Jazzy; the Humble live composes regenerate them against Humble before linking (`just e2e-h-ft` / CI `e2e-ros-talker-h-ft`, and the H-CY / H-ZN lanes in `e2e-ros-talker-humble-rmw`). SessionReady carries `adapter_abi_version` `serialized-adapter-v1`. `just ros-check` / `just ros-check-docker` compile `rclwebd --features ros --tests` without running them (CI `ros-feature-check`). `just ros-test` needs a sourced ROS environment matching those bindings (and packages such as `example_interfaces` for the live service and action loopbacks). Machines without apt ROS can use the optional RoboStack Jazzy prefix (`just ros-test-pixi`); that is not a toolchain pin and not a substitute for digest-pinned Docker e2e (`just e2e` / `just e2e-h-ft`). The J-FT runtime image (`just image-rclwebd`) is a separate deploy artifact from those e2e lanes.
